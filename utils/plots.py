...
def center_metrics(boxs):
    # boxs = [x0,y0,x1,y1]
    center_list = []
    for box in boxs:
        x0,y0,x1,y1 = box[0],box[1],box[2],box[4]
        center_x,center_y = int((x0+x1)/2),int((y0+y1)/2)
        center_list.append([center_x,center_y])

    center_list.sort(key=lambda ele: ele[0], reverse=False)
    center_xy0 = center_list[0]
    metrics = []
    for cent_xy in center_list[:1]:
        metric = sqrt((cent_xy[0] - center_xy0[0]) ** 2 + (cent_xy[1] - center_xy0[1]) ** 2)
        metrics.append(metric)

    return center_list,metrics
